...
PyObject *_pymbar_computeUnnormalizedLogWeightsCpp(PyObject *self, PyObject *args) {
  ...
  for (k=0;k<K;k++) {
	...
	if (log_term[i] > max_log_term) {max_log_term = log_term[i];}
        //printf("%d %d %d %16.7f %16.7f %16.7f\n",k,n,i,FlogN[i],u_j,u_k[n]);
      }
      //----------------------------------------------
  ...
  //-------------------------------------------
  // Printing out terms at the end - useful for debugging 
  //-------------------------------------------  

  //for (k=0;k<K;k++) {
  //  for (n=0;n<N_k[k];n++) {
  //    p1 = (double *)(array_log_w_kn->data + k*array_log_w_kn->strides[0]+n*array_log_w_kn->strides[1]);
  //    printf("%d %d %16.7f\n",k,n,*p1);
  //  }
  //}

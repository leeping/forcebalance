...
def run_simulation(pdb,settings,pbc=True,Trajectory=True):
   ...
   return data, xyzs, boxes, rhos, energies, simulation
...
def energy_derivatives(mvals,h,pdb,FF,xyzs,settings,simulation,boxes=None,AGrad=True):
   ...
   for i in range(FF.np):
      ...
      G[i,:] = f1d2p(fdwrap(energy_driver,mvals,i,key=None,pdb=pdb,FF=FF,xyzs=xyzs,settings=settings,simulation=simulation,boxes=boxes),h,f0=E0)
   return G, Hd
...
def main():
   ...
   #=================================================================#
   Data, Xyzs, Boxes, Rhos, Energies, Sim = run_simulation(pdb, mutual_kwargs if FF.amoeba_pol == 'mutual' else direct_kwargs, Trajectory=True)
   ...
   G, Hd = energy_derivatives(mvals, h, pdb, FF, Xyzs, mutual_kwargs if FF.amoeba_pol == 'mutual' else direct_kwargs, Sim, Boxes, AGrad)
   ...
   # Now that we have the coordinates, we can compute the energy derivatives.
   mG, mHd = energy_derivatives(mvals, h, mpdb, FF, mXyzs, mono_mutual_kwargs if FF.amoeba_pol == 'mutual' else mono_direct_kwargs, mSim, None, AGrad)

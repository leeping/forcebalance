...
gen_opts_types = {
    'strings' : {"gmxpath"      : (which('mdrun'), 60, 'Path for GROMACS executables (if not the default)', 'All targets that use GROMACS', ['GMX']),
                 ...
                 "amoeba_polarization"        : ('direct', 0, 'The AMOEBA polarization type, either direct or mutual.', 'Targets in OpenMM / TINKER that use the AMOEBA force field', ['OPENMM','TINKER'])
                 },
...
tgt_opts_types = {
    ...
    'ints'    : {"shots"              : (-1, 0, 'Number of snapshots; defaults to all of the snapshots', 'Energy + Force Matching', 'AbInitio'),
                 ...
                 "liquid_prod_steps"  : (100000, 0, 'Number of time steps for the liquid production run.', 'Condensed phase property targets', 'liquid'),
    ...
    'floats'  : {"weight"       : (1.0, 150, 'Weight of the target (determines its importance vs. other targets)', 'All targets (important)'),
                 ...
                 "energy_denom"   : (0.0, 0, 'Energy normalization for binding energies in kcal/mol (default is to use stdev)', 'Binding energy targets', 'binding'),
